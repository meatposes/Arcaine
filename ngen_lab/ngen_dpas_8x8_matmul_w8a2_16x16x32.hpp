// nGEN kernel-body authors for the fixed-shape w8a2 16x16x32 matmul.
...
constexpr size_t kABytes = kM * kK;            // 512 B, row-major s8
constexpr size_t kBBytes = kK * kN / 4;        // 128 B natural / packed
constexpr size_t kCBytes = kM * kN * 4;        // 1 KiB, row-major s32

// Tiles per thread (selected at runtime via NGEN_LAB_S2_TILES).
static int kTiles = 1;

// ---- register map (parametric in tiles-per-thread T) ----------------------
//   A: 8T GRFs starting at kA
//   B: 8 GRFs starting at kA + 8T (shared across tiles)
//   C: 16T GRFs starting at kA + 8T + 8
//   addresses/temps: fixed at the top of the GRF space
// For T=1: A 16-23, B 24-31, C 32-63.  T=2: A 16-31, B 32-39, C 40-71.
// T=4: A 16-47, B 48-55, C 56-119.  All fit within 128 GRFs.
constexpr int kA = 16;
...
inline int aBase() { return kA; }                 // A GRFs: kA .. kA+8T-1
inline int bBase() { return kA + 8 * kTiles; }    // B GRFs: +8
inline int cBase() { return kA + 8 * kTiles + 2; } // C GRFs: +16T
...
void setupTileAddresses(Generator &g, ngen::Subregister argA,
    ...
    // A rows are 32B each; C rows are 64B each.
    // rOffA and rOff are the SAME GRF, so capture rOffC = 64m BEFORE the
    // in-place halving (which would otherwise corrupt the C stride).
    GRF rOffA(kOff), rOffC(kOff + 1);
    g.mov(16, rOffC.ud(0)(1), rOff.ud(0)(1)); // C lane offset: 64*m
    g.shr(16, rOffA.ud(0)(1), rOff.ud(0)(1), 1); // A lane offset: 32*m
...
void loadATiles(Generator &g) {
    ...
    for (int t = 0; t < kTiles; t++) {
        GRF rPtrA(ptrA(t));
        for (int j = 0; j < 8; j++)
            g.load(16, GRF(aBase() + t * 8 + j), scattered(DataSizeLSC::D32, 1),
                    g.A64, rPtrA + 4 * j);
    }
}
...
void dpasKernelBody(Generator &g, ngen::Subregister argA,
    ...
    // Load packed B once (shared by all tiles): 2 N-halves x 4 GRFs.
    for (int h = 0; h < 2; h++)
        g.load(1, GRF(bBase() + h), block(DataSizeLSC::D32, 16), g.A64,
                rAddrB + h * 64);
    ...
    for (int t = 0; t < kTiles; t++)
        for (int h = 0; h < 2; h++) // N halves -> dst 8 GRFs apart
            g.dpas(16, 8, 8, GRF(cBase() + t * 16 + 8 * h).d(),
                    ...
                    GRF(aBase() + t * 8).b(), GRF(bBase() + h).s2());
