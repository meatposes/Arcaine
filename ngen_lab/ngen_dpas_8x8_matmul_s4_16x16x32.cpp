// nGEN-authored fixed-shape s8x4->s32 16x16x32 DPAS matmul microkernel.
//
...
// Mixed s8x4 dpas: sdepth 8 counts A dwords (4 s8/dword = K 32); B carries the
// same K with 8 s4/dword, so each N-half needs only 2 GRFs of B.
...
// ---- B packer -------------------------------------------------------------
// Packs natural row-major B[32][16] (s4) into the DPAS src2 VNNI layout:
//   piece h (N half 8h..8h+7), dword (j, r) =
//       {B[8j][8h+r], B[8j+1][8h+r], ..., B[8j+7][8h+r]}  (8 x 4-bit nibbles)
//   where j = K dword (0..3), r = column within half (0..7).
// Each half is 4x8 dwords = 32 dwords = 128B.
    ...
    for (int h = 0; h < 2; h++)
        for (int r = 0; r < 8; r++)
            for (int j = 0; j < 4; j++) {
                uint32_t d = 0;
                for (int e = 0; e < 8; e++) {
                    int8_t v = bNat[(8 * j + e) * kN + 8 * h + r];
                    d |= (uint32_t)(uint8_t)(v & 0xF) << (4 * e);
                }
                bPacked[(h * 32) + r * 4 + j] = d;
            }
}
...
int main() {
    ...
    std::uniform_int_distribution<int> distA(-128, 127); // A stays s8
    std::uniform_int_distribution<int> distB(-8, 7);     // B is s4
    ...
    std::printf("[correctness] int4 dpas (tiles=%d) mismatches=%zu/%zu -> %s\n",
    ...
    std::printf("[perf] int4 dpas (tiles=%d) groups=%u iters=%u  %8.3f ms  "
                "%8.2f GFLOP/s\n",
