// nGEN-authored fixed-shape s8x8->s32 16x16x32 DPAS matmul microkernel.
//
//   C[16x16] (s32) += A[16x32] (s8, row-major) * B[32x16] (s8)
//
// Same structure as the BF16 variant (ngen_dpas_8x8_matmul_bf16_16x16x32):
// SIMD16, lane m = row m of C. Tunable tiles-per-thread: NGEN_LAB_S8_TILES
...
int main() {
    ...
    kTiles = int(envU32("NGEN_LAB_S8_TILES", 1));
    ...
    if (kTiles < 1 || kTiles > 2) {
        std::fprintf(stderr, "NGEN_LAB_S8_TILES must be in [1,2]\n");
        return 1;
    }
    ...
    for (auto &v : hBnat) v = int8_t(dist(rng));
    ...
    std::printf("[correctness] int8 dpas (tiles=%d) mismatches=%zu/%zu -> %s\n",
    ...
    std::printf("[perf] int8 dpas (tiles=%d) groups=%u iters=%u  %8.3f ms  "
                "%8.2f GFLOP/s\n",
