// nGEN-authored fixed-shape s4x8->s32 (w4a8) 16x16x32 DPAS matmul microkernel.
//
//   C[16x16] (s32) += A[16x32] (s8, row-major; B is s4) * B[32x16] (s8)
//
// Same structure as the BF16 variant (ngen_dpas_8x8_matmul_bf16_16x16x32):
// SIMD16, lane m = row m of C. Tunable tiles-per-thread: NGEN_LAB_S4_TILES
...
//   B operand (src2, broadcast, s4): dword (j, r) = {B[8j][r]..B[8j+7][r]}
//                                 (8 s4 per dword along K, VNNI). packB
//                                 produces 2 N-halves of 128B each; loaded as
//                                 contiguous 64B block chunks.
//   C accumulator (dst/src0):     GRF n, lane m = C[m][n] (s32).
// Both A and B are s4 (8 values/dword). A: K=32 -> 4 dwords/row -> 4 GRFs.
// B: 8 K/dword -> 4 dwords/column -> 2 GRFs per N-half.
    ...
    for (int h = 0; h < 2; h++)
        for (int r = 0; r < 8; r++)
            for (int j = 0; j < 8; j++) {
                uint32_t d = 0;
                for (int e = 0; e < 4; e++) {
                    int8_t v = bNat[(4 * j + e) * kN + 8 * h + r];
                    d |= (uint32_t)(uint8_t)v << (8 * e);
                }
                bPacked[(h * 64) + r * 8 + j] = d;
            }
}

// ---- A packer --------------------------------------------------------------
// Packs row-major A[16][32] (s4, int8-hosted values) into nibble-packed bytes:
//   8 s4 values per dword, low nibble = lowest K index.
static void packA(const int8_t *aNat, size_t tiles, uint32_t *aPacked) {
    for (size_t t = 0; t < tiles; t++)
        for (int m = 0; m < kM; m++)
            for (int j = 0; j < 4; j++) {
                uint32_t d = 0;
                for (int e = 0; e < 8; e++) {
                    int8_t v = aNat[t * kM * kK + m * kK + 8 * j + e];
                    d |= (uint32_t)(uint8_t)(v & 0xF) << (4 * e);
                }
                aPacked[t * (kM * 4) + m * 4 + j] = d;
            }
}
...
int main() {
    ...
    kTiles = int(envU32("NGEN_LAB_S4_TILES", 1));
    ...
    if (kTiles < 1 || kTiles > 2) {
        std::fprintf(stderr, "NGEN_LAB_S4_TILES must be in [1,2]\n");
        return 1;
    }
    ...
    std::uniform_int_distribution<int> distA(-8, 7);     // A is s4
    std::uniform_int_distribution<int> distB(-128, 127); // B is s8
    ...
    std::vector<uint32_t> hApacked((size_t)maxGroups * kM * 4);
    packA(hA.data(), maxGroups, hApacked.data());

    L0 l0;
    void *dA = l0.alloc((size_t)maxGroups * kABytes);
    void *dBpacked = l0.alloc(kBBytes);
    void *dC = l0.alloc((size_t)maxGroups * kCBytes);
    std::memcpy(dA, hApacked.data(), (size_t)maxGroups * kABytes);
    ...
    std::printf("[correctness] w4a8 dpas (tiles=%d) mismatches=%zu/%zu -> %s\n",
    ...
    std::printf("[perf] w4a8 dpas (tiles=%d) groups=%u iters=%u  %8.3f ms  "
                "%8.2f GFLOP/s\n",
