// nGEN kernel-body authors for the fixed-shape s4 16x16x32 matmul.
...
//   B: 4 GRFs starting at kA + 8T (shared across tiles; 2 per N-half)
//   C: 16T GRFs starting at kA + 8T + 4
...
inline int bBase() { return kA + 8 * kTiles; }    // B GRFs: +4 (2/half)
inline int cBase() { return kA + 8 * kTiles + 4; } // C GRFs: +16T
...
void dpasKernelBody(Generator &g, ngen::Subregister argA,
    ...
    // Load packed B once (shared by all tiles): 2 N-halves x 2 GRFs.
    // Each half is 2 GRFs (64 dwords of s4 = 128B).
    for (int h = 0; h < 2; h++)
        for (int j = 0; j < 2; j++)
            g.load(1, GRF(bBase() + 2 * h + j), block(DataSizeLSC::D32, 16), g.A64,
                    rAddrB + (h * 128 + j * 64));
    ...
    for (int t = 0; t < kTiles; t++)
        for (int h = 0; h < 2; h++) // N halves -> dst 8 GRFs apart
            g.dpas(16, 8, 8, GRF(cBase() + t * 16 + 8 * h).d(),
                    ...
                    GRF(aBase() + t * 8).b(), GRF(bBase() + 2 * h).s4());
