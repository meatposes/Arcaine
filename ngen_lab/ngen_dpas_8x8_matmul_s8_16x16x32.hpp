// nGEN kernel-body authors for the fixed-shape s8 16x16x32 matmul.
...
// Tiles per thread (selected at runtime via NGEN_LAB_S8_TILES).
static int kTiles = 1;
...
inline int cBase() { return kA + 8 * kTiles + 8; } // C GRFs: +16T
    ...
    for (int t = 0; t < kTiles; t++)
        for (int h = 0; h < 2; h++) // N halves -> dst 8 GRFs apart
            g.dpas(16, 8, 8, GRF(cBase() + t * 16 + 8 * h).d(),
                    ...
                    GRF(aBase() + t * 8).b(), GRF(bBase() + 4 * h).b());
