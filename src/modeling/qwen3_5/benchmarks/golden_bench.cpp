// qwen3_5 — numerical gate for the kernel A/B flags.
//
// Every fast path in this model is selected by an environment flag
// (ARCAINE_QWEN35_NVFP4_DPAS, ARCAINE_QWEN35_XMX_ATTENTION,
// ARCAINE_QWEN35_ESIMD_DELTA, ARCAINE_QWEN35_FUSED_*, ...). Timing them against
// each other only means something if they compute the same thing, and a
// benchmark cannot tell a kernel that is fast from a kernel that is fast and
// wrong. The existing kernel benchmarks check shapes and pointers against
// random weights, which catches wiring but not arithmetic.
//
// `capture` records the logit trajectory of a reference configuration;
// `compare` replays the identical token sequence under whatever configuration
// is currently selected and reports how far it drifted.
//
// Both are teacher-forced — every step is fed the recorded token rather than
// its own sample — so the comparison does not depend on the sampler, and so
// perplexity over the same tokens is directly comparable between runs. That
// last number is the gate for changes that are *intentionally* lossy (a lower
// precision head, quantized activations): max-abs-diff is large by design there
// and the real question is whether the model got worse.
//
//   ./build/arcaine_mbench --model DIR --golden capture --out FILE [opts]
//   ./build/arcaine_mbench --model DIR --golden compare --golden-file FILE [opts]
//
// It lives with the model benchmarks rather than the kernel benchmarks because
// it drives the concrete engine; the kernel-bench binary links no model.
//
// Typical use, capture with the conservative kernels and compare with the fast
// ones:
...
//     ./build/arcaine_mbench --model $M --golden capture --out golden.bin
//   ./build/arcaine_mbench --model $M --golden compare --golden-file golden.bin
...
const char* const kTrackedEnv[] = {
    ...
    "ARCAINE_QWEN35_FUSED_DECODE_MAX_SEQ",
...
bool write_golden(const std::string& path, const Golden& g) {
    ...
    u32(kVersion); u32(g.vocab); u32(g.prefill); u32(g.records);
...
bool read_golden(const std::string& path, Golden& g) {
    ...
    if (std::memcmp(magic, kMagic, sizeof magic) != 0) {
        std::fprintf(stderr, "%s: not a golden file\n", path.c_str());
        return false;
    }
    auto u32 = [&] { uint32_t v = 0; f.read((char*)&v, 4); return v; };
    if (u32() != kVersion) {
        std::fprintf(stderr, "%s: golden format version mismatch\n", path.c_str());
        return false;
    }
    g.vocab = u32(); g.prefill = u32(); g.records = u32();
...
// Against the max, so a 248k-wide logit vector does not overflow.
...
// Mean negative log-likelihood of each recorded target. Meaningful only when
// the tokens came from real text.
...
// One prefill over the first `prefill` tokens, then one single-token decode per
// remaining position. Record r is the logit vector predicting
// tokens[prefill + r].
std::vector<float> run_trajectory(Qwen35Model& model,
    ...
    for (uint32_t r = 1; r < records; ++r) {
        ...
        std::copy(logits.begin(), logits.end(), out.begin() + (size_t)r * vocab);
...
const char* USAGE =
    "Usage:\n"
    "  arcaine_kbench qwen35-golden capture --model DIR --out FILE [opts]\n"
    "  arcaine_kbench qwen35-golden compare --model DIR --golden FILE [opts]\n"
    "\n"
...
int run(int argc, char** argv) {
    if (argc < 2) { std::fputs(USAGE, stderr); return 1; }
    std::string mode = argv[1];
    if (mode == "-h" || mode == "--help") { std::fputs(USAGE, stderr); return 0; }
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        else if (a == "--golden-file") golden_path = next();
        ...
        else if (a == "-h" || a == "--help") { std::fputs(USAGE, stderr); return 0; }
        else { std::fprintf(stderr, "unknown argument: %s\n", a.c_str()); return 1; }
    }
    if (model_dir.empty()) { std::fputs(USAGE, stderr); return 1; }
    if (mode == "capture" && out_path.empty()) { std::fputs(USAGE, stderr); return 1; }
    if (mode == "compare" && golden_path.empty()) { std::fputs(USAGE, stderr); return 1; }
    if (steps < 1 || prefill < 1) {
        std::fputs("--steps and --prefill must be positive\n", stderr);
        return 1;
    }
    if (device_set) gpu_device_control::apply_device_index(device);
    ...
    // The golden run fixes the sequence length, so the cache must be deep
    // enough to replay it.
    int needed = (mode == "compare") ? (int)golden.tokens.size() + 1
                                     : prefill + steps + 1;
    ...
    if (mode == "compare") {
        if (golden.vocab != vocab) {
            std::fprintf(stderr, "golden vocab %u != model vocab %u\n",
                         golden.vocab, vocab);
            return 1;
        }
    ...
    if ((int)tokens.size() < prefill + steps) {
        if (mode == "compare") { std::fputs("golden file is truncated\n", stderr); return 1; }
        steps = (int)tokens.size() - prefill;
        if (steps < 1) {
            std::fprintf(stderr, "token source has %d tokens, need more than --prefill %d\n",
                         (int)tokens.size(), prefill);
            return 1;
        }
        std::printf("[golden] token source is short; capturing %d records\n", steps);
    }
    tokens.resize(prefill + steps);

    std::printf("[golden] prefill %d, %d records, vocab %u\n", prefill, steps, vocab);
    std::printf("[golden] config: %s\n", env_snapshot().c_str());
    ...
    if (mode == "capture") {
        Golden g;
        g.vocab = vocab; g.prefill = (uint32_t)prefill; g.records = (uint32_t)steps;
        g.config = env_snapshot(); g.tokens = tokens; g.logits = std::move(logits);
        if (!write_golden(out_path, g)) {
            std::fprintf(stderr, "failed to write %s\n", out_path.c_str());
            return 1;
        }
        double ppl = perplexity(g.logits, g.records, vocab, tokens, (uint32_t)prefill);
        std::printf("[golden] wrote %s (%.1f MiB)\n", out_path.c_str(),
                    (double)g.logits.size() * 4 / (1 << 20));
        std::printf("[golden] perplexity %.4f%s\n", ppl,
                    real_text ? "" : "  (synthetic tokens - not meaningful)");
        return 0;
    }

    double max_abs = 0.0, sum_abs = 0.0, max_kl = 0.0, sum_kl = 0.0;
    int max_abs_step = -1;
    size_t agree = 0;
    for (uint32_t r = 0; r < (uint32_t)steps; ++r) {
        ...
        // KL(golden || current): the distributional distance that tracks output
        // quality, unlike a max over raw logits which one irrelevant token can
        // dominate.
        double lse_a = logsumexp(a, (int)vocab), lse_b = logsumexp(b, (int)vocab);
        double kl = 0.0;
        for (uint32_t v = 0; v < vocab; ++v) {
    ...
    double ppl_golden = perplexity(golden.logits, (uint32_t)steps, vocab, tokens,
                                   (uint32_t)prefill);
    double ppl_current = perplexity(logits, (uint32_t)steps, vocab, tokens,
                                    (uint32_t)prefill);

    std::printf("\ngolden config : %s\n", golden.config.c_str());
    std::printf("current config: %s\n\n", env_snapshot().c_str());
    std::printf("  records            %d\n", steps);
    std::printf("  max |dlogit|       %.6g  (step %d)\n", max_abs, max_abs_step);
    std::printf("  mean |dlogit|      %.6g\n", sum_abs / ((double)steps * vocab));
    ...
    bool ok = true;
    if (max_abs > tol_abs) {
        std::printf("\nFAIL: max |dlogit| %.6g exceeds --tol-abs %.6g\n", max_abs, tol_abs);
        ok = false;
    }
    if (top1 < tol_top1) {
...
// Entry point for the qwen3_5 model benchmark's --golden mode. argv[1] is the
// subcommand (capture or compare), matching the shape run() already expects.
int run_golden(int argc, char** argv) { return run(argc, argv); }
